...
//
//   - Local Whetstone password (legacy): users who predate xors
...
export const authRoutes = new Elysia({ prefix: "/auth" })
	.post(
		"/login",
		async ({ body, set }) => {
			// Local-first. Existing Whetstone users keep working without
	...
	// consumed against the local Whetstone auth path.
	.get("/recovery-codes/status", async ({ request, set }) => {
		const token = readSessionToken(request.headers);
		const user = await getUserBySession(token);
		if (!user) {
			set.status = 401;
			return { error: "Sign in with your Whetstone password to view recovery code status." };
		}
	...
	.post("/recovery-codes/generate", async ({ request, set }) => {
		...
		if (!user) {
			set.status = 401;
			return { error: "Sign in with your Whetstone password to generate recovery codes." };
		}
